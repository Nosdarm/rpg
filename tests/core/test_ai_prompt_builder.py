...
import json
import unittest
from unittest.mock import patch, AsyncMock, MagicMock # Added MagicMock for sync functions

from sqlalchemy.ext.asyncio import AsyncSession

# Assuming models are correctly imported in the module being tested
from src.models import GuildConfig, Location, Player, GeneratedNpc, Relationship, RuleConfig # Added RuleConfig
...
from src.models.enums import LocationType, RelationshipEntityType # Added RelationshipEntityType


# Mocking CRUD placeholders that might be used by the functions under test if not replaced by actuals
# This is to ensure tests can run even if some CRUDs are still placeholders in ai_prompt_builder.
# Ideally, these would be actual mocks of actual CRUDs.

# Simplified mock for guild_config_crud.get
async def mock_get_guild_config(session, id, guild_id=None): # Added guild_id default
    if id == 1 and (guild_id is None or guild_id == 1) : # Added guild_id check
        mock_gc = GuildConfig(id=1, main_language="en")
        return mock_gc
    return None

# Simplified mock for location_crud.get
async def mock_get_location(session, id, guild_id=None):
    if id == 1 and guild_id == 1: # Example specific mock
        ...
        return mock_loc
    if id == 100 and guild_id == 1: # For dialogue test
...
# Mock for generated_npc_crud.get and get_multi_by_attribute
async def mock_get_npc_generic(session, id, guild_id=None): # For .get
    if id == 10 and guild_id == 1: # For dialogue test
...
async def mock_get_multi_npc_generic(session, guild_id, **kwargs): # For .get_multi_by_attribute
    if guild_id == 1 and kwargs.get("current_location_id") == 100: # For dialogue test, no other NPCs
        return []
    return []
...
class TestAIPromptBuilder(unittest.IsolatedAsyncioTestCase):
    ...
    @patch('src.core.ai_prompt_builder.guild_config_crud.get', side_effect=mock_get_guild_config)
    @patch('src.core.ai_prompt_builder.location_crud.get', side_effect=mock_get_location)
    @patch('src.core.ai_prompt_builder.player_crud.get', new_callable=AsyncMock) # General mock for player
    @patch('src.core.ai_prompt_builder.party_crud.get', new_callable=AsyncMock) # General mock for party
    @patch('src.core.ai_prompt_builder.generated_npc_crud.get_multi_by_attribute', side_effect=mock_get_multi_npc_generic)
    @patch('src.core.ai_prompt_builder.crud_relationship.get_relationship', new_callable=AsyncMock) # Assuming this is the name for relationship_crud.get_relationship
    ...
    async def test_prepare_ai_prompt_basic_scenario(
        self, mock_get_all_rules, mock_get_relationship, mock_get_nearby_npcs,
        mock_party_get, mock_player_get, mock_location_get, mock_guild_get
    ):
        mock_session = AsyncMock(spec=AsyncSession)

        # Setup mocks
        mock_guild_get.return_value = GuildConfig(id=1, main_language="en")
        # mock_location_get is handled by side_effect

        mock_player_get.return_value = Player(id=1, guild_id=1, name="TestPlayer", level=5, xp=100, current_status="active")
        mock_party_get.return_value = None # No party in this scenario
        # mock_get_nearby_npcs side_effect handles this
        mock_get_relationship.return_value = None # No specific relationships

        # Mock for get_all_rules_for_guild to return a dictionary
        mock_get_all_rules.return_value = {
            "guild_main_language": "en",
            "ai_generation_style": "dark_fantasy",
            "world_description_i18n": {"en": "A vast and perilous world."} # Example rule
        }
        ...
        result_prompt = await prepare_ai_prompt(
            ...
            guild_id=1,
            location_id=1, # Uses mock_get_location for this
            player_id=1,
            party_id=None
        )
        ...
        self.assertIn("## AI Content Generation Request", result_prompt)
        self.assertIn("Target Guild ID: 1", result_prompt)
        self.assertIn("Primary Language for Generation: en", result_prompt)
        self.assertIn("Location: Test Loc", result_prompt)
        self.assertIn("Player: TestPlayer (Lvl 5, Status: active)", result_prompt) # Updated assertion
        self.assertIn("Nearby NPCs: None notable", result_prompt) # Based on mock_get_multi_npc_generic
        self.assertIn("World State: A vast and perilous world.", result_prompt) # From mocked world_description_i18n
        self.assertIn("Generation Style: dark_fantasy", result_prompt) # From mocked rules
        self.assertIn("npc_schema", result_prompt) # Check for schema inclusion
        self.assertIn("quest_schema", result_prompt)


    @patch('src.core.ai_prompt_builder.guild_config_crud.get', side_effect=mock_get_guild_config)
    @patch('src.core.ai_prompt_builder.location_crud.get', new_callable=AsyncMock)
    async def test_prepare_ai_prompt_location_not_found(self, mock_location_get, mock_guild_get):
        mock_session = AsyncMock(spec=AsyncSession)
        mock_guild_get.return_value = GuildConfig(id=1, main_language="en")
        mock_location_get.return_value = None # Location not found

        result_prompt = await prepare_ai_prompt(mock_session, guild_id=1, location_id=999)

        # Depending on implementation, it might return an error string or a very minimal prompt.
        # The current implementation logs a warning and continues, potentially leading to a less useful prompt.
        # Let's check if it still tries to generate something or indicates an issue.
        # Based on the code: "Location: N/A - Not specified."
        self.assertIn("Location: N/A", result_prompt)
        self.assertNotIn("Error: Location not found", result_prompt) # The function itself doesn't return this error string
        self.assertIn("Generate enriching content", result_prompt) # It still tries to generate for "this location"
...
class TestAIFactionPromptBuilder(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_prepare_faction_relationship_generation_prompt_basic(
        ...
        self.assertIn("## AI Faction & Relationship Generation", prompt)
        self.assertIn("Target Guild ID: 1", prompt)
        self.assertIn("Primary Language for Generation: ru", prompt)
        self.assertIn("World: Мир на грани войны", prompt)
        self.assertIn("Target Number of Factions to Generate: 2", prompt)
        self.assertIn("Торговые гильдии", prompt)
        self.assertIn("Рыцарские ордена", prompt)
        self.assertIn("Relationship Complexity: low", prompt)
...
class TestAIQuestPromptBuilder(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_prepare_quest_generation_prompt_with_context(
        ...
        self.assertIn("## AI Quest Generation", prompt)
        self.assertIn("Target Guild ID: 1", prompt)
        ...
        self.assertIn("Target Number of Quests to Generate: 1", prompt)
...
class TestAIEconomicPromptBuilder(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_prepare_economic_entity_generation_prompt_basic(
        ...
        mock_get_all_rules.return_value = {
            ...
            "guild_main_language": "en" # Needed by _get_game_rules_terms indirectly
        }
        ...
        self.assertIn("## AI Economic Entity Generation Request", result)
        self.assertIn("Target Guild ID: 1", result)
        self.assertIn("Language for NPC's response: en", result)
        self.assertIn("item_schema", result)
        self.assertIn("npc_trader_schema", result)
        self.assertIn("npc_schema", result)
        self.assertIn("World Description: A test world where items are scarce.", result)
        self.assertIn("Target Number of New Items to Generate: 3", result)
        self.assertIn("Target Number of New NPC Traders to Generate: 1", result)
...
class TestDialoguePromptBuilder(unittest.IsolatedAsyncioTestCase):
    ...
    @patch('src.core.ai_prompt_builder._get_guild_main_language', new_callable=AsyncMock)
    @patch('src.core.ai_prompt_builder.actual_npc_crud.get', new_callable=AsyncMock)
    ...
    @patch('src.core.ai_prompt_builder.actual_crud_relationship.get_relationship_between_entities', new_callable=AsyncMock)
    ...
    async def test_prepare_dialogue_prompt_basic_success(
        ...
        # Mocks for other context functions to return neutral/empty but valid values
    ...
    @patch('src.core.ai_prompt_builder.actual_npc_crud.get', new_callable=AsyncMock)
    @patch('src.core.ai_prompt_builder._get_guild_main_language', new_callable=AsyncMock)
    async def test_prepare_dialogue_prompt_npc_not_found(self, mock_get_lang, mock_get_npc):
        mock_session = AsyncMock(spec=AsyncSession)
        mock_get_lang.return_value = "en"
        mock_get_npc.return_value = None
    ...
    async def test_dialogue_history_inclusion(
        ...
        # Neutral/empty returns for other context functions
