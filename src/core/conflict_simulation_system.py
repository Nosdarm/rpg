...
from src.models.actions import ParsedAction, ActionEntity # Added ActionEntity
...
class PydanticConflictForSim(BaseModel): # Renamed
    """
    A Pydantic model to represent a simulated pending conflict,
    not necessarily matching the DB schema directly but useful for simulation logic.
    """
    guild_id: int
    conflict_type: str
    status: ConflictStatus
    involved_entities_json: List[Dict[str, Any]]
    resolution_details_json: Dict[str, Any] = {} # For storing target_signature, intents etc.
    turn_number: int # Simulation might operate on a hypothetical turn

    # Optional: include conflicting actions if needed for output, similar to SA model
...
async def simulate_conflict_detection(
...
) -> List[PydanticConflictForSim]: # Changed return type
    """
    ...
    try:
        ...
        for action_data in actions_input_data:
            ...
            raw_parsed_action_dict = action_data.get("parsed_action") # Renamed to avoid confusion

            if not all([actor_id, actor_type_str, raw_parsed_action_dict]):
                logger.warning(f"Skipping invalid action data in simulation (missing actor_id, actor_type, or parsed_action): {action_data}")
                continue

            # Assertions to help pyright understand that these are not None
            assert actor_id is not None, "actor_id should not be None here"
            assert actor_type_str is not None, "actor_type_str should not be None here"
            assert raw_parsed_action_dict is not None, "raw_parsed_action_dict should not be None here"

            action_intent = "unknown_intent"
            action_raw_text = ""
            action_entities_dicts: List[Dict[str, Any]] = [] # Initialized
            parsed_entities: List[ActionEntity] = [] # Initialized for logger in except block
            try:
                # Prepare arguments for ParsedAction
                action_intent = raw_parsed_action_dict.get("intent_name", "unknown_intent")
                action_raw_text = raw_parsed_action_dict.get("text", "")
                action_entities_dicts = raw_parsed_action_dict.get("entities", []) # List[Dict]

                # Explicitly convert to List[ActionEntity] for Pyright
                # parsed_entities: List[ActionEntity] = [] # Moved initialization up
                    ...
                    for entity_dict in action_entities_dicts:
                        if isinstance(entity_dict, dict):
                            try:
                                ...
                                # Decide: skip this entity, or fail the action? For now, skip entity.
                        else:
                ...
                else: # For NPCs or other types
                    logger.warning(f"NPC or non-player action for actor {actor_id} in guild {guild_id}. Using placeholder player_id 0.")
                    current_player_id = 0

                parsed_action = ParsedAction(
                    raw_text=action_raw_text,
                    intent=action_intent,
                    entities=parsed_entities, # Now explicitly List[ActionEntity]
                    guild_id=guild_id,
                    player_id=current_player_id
                )
            except Exception as e:
                logger.warning(f"Skipping action data due to ParsedAction validation error: {e} - Input dict: {raw_parsed_action_dict}, Parsed args: raw_text='{action_raw_text}', intent='{action_intent}', entities={action_entities_dicts}, Guild: {guild_id}, Actor: {actor_id}")
                continue
        ...
        # simulated_pending_conflicts: List[PendingConflict] = [] # Removed this older declaration
        ...
        def _extract_primary_target_signature(action: ParsedAction) -> Optional[str]:
            ...
            def find_entity_value(etype: str) -> Optional[str]: # Ensured Optional[str]
                for entity in entities:
                    if entity.type == etype:
                        return entity.value # entity.value is str
                return None
            ...
            if intent in ["attack", "talk", "trade_view_inventory", "trade_buy_item", "trade_sell_item"]:
                ...
                if target_npc_id is not None: return f"npc:{target_npc_id}"
                target_player_id = find_entity_value("target_player_id")
                if target_player_id is not None: return f"player:{target_player_id}"
                target_npc_name = find_entity_value("target_npc_name")
                if target_npc_name is not None: return f"npc_name:{target_npc_name.lower()}"
            ...
            if intent in ["take", "drop"] or (intent == "examine" and any(e.type in ["target_item_id", "item_static_id", "item_name"] for e in entities)):
                ...
                if target_item_id is not None: return f"item_instance:{target_item_id}"
                item_static_id = find_entity_value("item_static_id")
                if item_static_id is not None: return f"item_static:{item_static_id}"
                item_name = find_entity_value("item_name") # Less precise
                if item_name is not None: return f"item_name:{item_name.lower()}"
            ...
            if intent == "interact" or (intent == "examine" and any(e.type in ["target_object_static_id", "target_object_name"] for e in entities)):
                ...
                if obj_static_id is not None: return f"obj_static:{obj_static_id}"
                obj_name = find_entity_value("target_object_name")
                if obj_name is not None: return f"obj_name:{obj_name.lower()}"
            ...
            if intent == "use":
                ...
                if item_id_val is not None: # Check item_id_val before using it for find_entity_value("target_item_id")
                    # The original logic was: f"item_instance:{item_id_val}" if find_entity_value("target_item_id") else f"item_static:{item_id_val}"
                    # This implies item_id_val could be from target_item_id or item_static_id.
                    # We need to know which one it was to correctly form the signature.
                    # Let's refine this:
                    target_item_id_for_use = find_entity_value("target_item_id")
                    item_static_id_for_use = find_entity_value("item_static_id")
                    if target_item_id_for_use is not None:
                        item_signature_part = f"item_instance:{target_item_id_for_use}"
                    elif item_static_id_for_use is not None:
                        item_signature_part = f"item_static:{item_static_id_for_use}"
                ...
                if item_signature_part: # Primarily item-focused 'use'
                    target_npc_id = find_entity_value("target_npc_id")
                    if target_npc_id is not None: return f"use:{item_signature_part}@target:npc:{target_npc_id}"
                    target_player_id = find_entity_value("target_player_id")
                    if target_player_id is not None: return f"use:{item_signature_part}@target:player:{target_player_id}"
                ...
                skill_id_val = find_entity_value("skill_id") # Prefer ID
                skill_name_val = find_entity_value("skill_name")

                actual_skill_ref = None
                if skill_id_val is not None:
                    actual_skill_ref = skill_id_val
                elif skill_name_val is not None:
                    actual_skill_ref = skill_name_val.lower()

                if actual_skill_ref is not None:
                    skill_sig_part = f"skill:{actual_skill_ref}"
                    target_npc_id = find_entity_value("target_npc_id")
                    if target_npc_id is not None: return f"use:{skill_sig_part}@target:npc:{target_npc_id}"
                    target_player_id = find_entity_value("target_player_id")
                    if target_player_id is not None: return f"use:{skill_sig_part}@target:player:{target_player_id}"
                    return f"use_on_self:{skill_sig_part}" # Default to self if no other target

            # 5. Movement Intent
            if intent == "move":
                loc_static_id = find_entity_value("location_static_id")
                if loc_static_id is not None: return f"location_static:{loc_static_id}"
                loc_name = find_entity_value("location_name")
                if loc_name is not None: return f"location_name:{loc_name.lower()}"
                direction = find_entity_value("direction")
                if direction is not None: return f"direction:{direction.lower()}" # Can still conflict
            ...
            if len(entities) == 1:
                entity = entities[0]
                # Ensure entity.value is not None before lower() though ActionEntity.value is str
                if entity.value is not None and \
        ...
        simulated_pending_conflicts: List[PydanticConflictForSim] = [] # Changed type
        ...
        for target_sig, actions_on_this_target in actions_by_target_sig.items():
            ...
            if len(attack_actions) > 1:
                ...
                sim_conflict = PydanticConflictForSim( # Renamed
                    guild_id=guild_id,
                    ...
                    if intent_pair in CONFLICTING_INTENT_PAIRS_ON_SAME_TARGET:
                        ...
                        for conflict_name_tpl, target_category_pattern in rules_for_pair:
                            if target_sig.startswith(target_category_pattern):
                                ...
                                # Construct a conflict_type that includes all necessary info for potential deduplication
                                current_conflict_type = f"sim_{conflict_name_tpl}_on_{target_sig.replace(':', '_')}_with_intents_{'_'.join(sorted(list(intent_pair)))}"
                                ...
                                is_duplicate = False
                                for ec_loop_var in simulated_pending_conflicts:
                                    existing_conflict: PydanticConflictForSim = ec_loop_var # Renamed
                                    existing_actors_involved = frozenset((e["entity_id"], e["action_intent"]) for e in existing_conflict.involved_entities_json)
                                    current_actors_involved = frozenset(((actor1_sim.id, action1.intent), (actor2_sim.id, action2.intent)))

                                    # Check if conflict_type and the set of involved actors/intents match
                                    # And also check the target signature from resolution_details_json
                                    if existing_conflict.conflict_type == current_conflict_type and \
                                       existing_actors_involved == current_actors_involved and \
        ...
        for u_action_info in use_on_self_actions:
            ...
            if item_being_used_sig in take_actions_map:
                for t_actor_sim, t_action in take_actions_map[item_being_used_sig]:
                    ...
                    # TODO: Add deduplication for this conflict type as well if necessary, similar to above.
                    sim_conflict = PydanticConflictForSim( # Renamed
                        guild_id=guild_id,
                    ...
                    simulated_pending_conflicts.append(sim_conflict) # Add deduplication if needed
