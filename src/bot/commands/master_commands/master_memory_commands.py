...
class MasterMemoryCog(commands.Cog, name="Master Memory Commands"):
    ...
    async def memory_view(self, interaction: discord.Interaction, memory_id: int):
        ...
        async with get_db_session() as session:
            ...
            if not memory_entry:
                not_found_msg = await get_localized_message_template(session, interaction.guild_id, "memory_view:not_found", lang_code, "Memory entry ID {id} not found.") # type: ignore
            ...
            title_template = await get_localized_message_template(session, interaction.guild_id, "memory_view:title", lang_code, "Memory: {p_name} <> {n_name} (ID: {id})") # type: ignore
            ...
            async def get_label(key: str, default: str) -> str:
                return await get_localized_message_template(session, interaction.guild_id, f"memory_view:label_{key}", lang_code, default) # type: ignore
            ...
            ts_val = discord.utils.format_dt(memory_entry.timestamp, style='F') if memory_entry.timestamp else na_value_str
            embed.add_field(name=await get_label("timestamp", "Timestamp"), value=ts_val, inline=False) # type: ignore
    ...
    async def memory_list(self, interaction: discord.Interaction,
        ...
        async with get_db_session() as session:
            ...
            if event_type: conditions.append(PlayerNpcMemory.event_type.ilike(f"%{event_type}%")) # type: ignore

            offset = (page - 1) * limit
            query = select(PlayerNpcMemory).where(and_(*conditions)).offset(offset).limit(limit).order_by(PlayerNpcMemory.timestamp.desc()) # type: ignore
            ...
            player_id_filter_label = await get_localized_message_template(session, interaction.guild_id, "memory_list:filter_player_id", lang_code, "Player ID") # type: ignore
            npc_id_filter_label = await get_localized_message_template(session, interaction.guild_id, "memory_list:filter_npc_id", lang_code, "NPC ID") # type: ignore
            event_filter_label = await get_localized_message_template(session, interaction.guild_id, "memory_list:filter_event", lang_code, "Event") # type: ignore
            all_filter_label = await get_localized_message_template(session, interaction.guild_id, "common:filter_all", lang_code, "All") # type: ignore
            ...
            if not mem_entries:
                no_mem_msg = await get_localized_message_template(session,interaction.guild_id,"memory_list:no_entries_found",lang_code,"No Memory entries for {filter} (Page {p}).") # type: ignore
                await interaction.followup.send(no_mem_msg.format(filter=filter_display, p=page), ephemeral=True); return

            title_tmpl = await get_localized_message_template(session,interaction.guild_id,"memory_list:title",lang_code,"Memory List ({filter} - Page {p} of {tp})") # type: ignore
            ...
            footer_tmpl = await get_localized_message_template(session,interaction.guild_id,"memory_list:footer",lang_code,"Displaying {c} of {t} total entries.") # type: ignore
            embed.set_footer(text=footer_tmpl.format(c=len(mem_entries), t=total_mem_entries))
            name_tmpl = await get_localized_message_template(session,interaction.guild_id,"memory_list:entry_name_field",lang_code,"ID: {id} | Player {p_id} <> NPC {n_id}") # type: ignore
            val_tmpl = await get_localized_message_template(session,interaction.guild_id,"memory_list:entry_value_field",lang_code,"Event: {e_type} (@ {ts})") # type: ignore

            for entry in mem_entries:
                na_value_str = await get_localized_message_template(session, interaction.guild_id, "common:value_na", lang_code, "N/A") # type: ignore
                ts_formatted = discord.utils.format_dt(entry.timestamp, style='R') if entry.timestamp else na_value_str # type: ignore
                embed.add_field(
    ...
    async def memory_delete(self, interaction: discord.Interaction, memory_id: int):
        ...
        async with get_db_session() as session:
            ...
            if not memory_to_delete:
                error_msg = await get_localized_message_template(session,interaction.guild_id,"memory_delete:error_not_found",lang_code,"Memory entry ID {id} not found.") # type: ignore
            ...
            try:
                ...
                if deleted_entry:
                    success_msg = await get_localized_message_template(session,interaction.guild_id,"memory_delete:success",lang_code,"Memory entry ID {id} ({repr}) deleted.") # type: ignore
                    await interaction.followup.send(success_msg.format(id=memory_id, repr=entry_repr), ephemeral=True)
                else:
                    error_msg = await get_localized_message_template(session,interaction.guild_id,"memory_delete:error_unknown_fail",lang_code,"Memory entry (ID: {id}) found but not deleted.") # type: ignore
                ...
                error_msg = await get_localized_message_template(session,interaction.guild_id,"memory_delete:error_generic_delete",lang_code,"Error deleting Memory ID {id}: {err}") # type: ignore
