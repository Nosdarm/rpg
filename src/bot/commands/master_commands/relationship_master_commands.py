    ...
    async def relationship_view(self, interaction: discord.Interaction, relationship_id: int):
        ...
        async with get_db_session() as session:
            ...
            async def get_label(key: str, default: str) -> str:
                return await get_localized_message_template(session, interaction.guild_id, f"relationship_view:label_{key}", lang_code, default) # type: ignore
        ...
        async with get_db_session() as session:
            if entity1_type:
                try: e1_type_enum = RelationshipEntityType[entity1_type.upper()]
                    ...
                    error_msg = await get_localized_message_template(session, interaction.guild_id, "relationship_list:error_invalid_e1_type", lang_code, "Invalid entity1_type.") # type: ignore
            ...
            if entity2_type:
                try: e2_type_enum = RelationshipEntityType[entity2_type.upper()]
                    ...
                    error_msg = await get_localized_message_template(session, interaction.guild_id, "relationship_list:error_invalid_e2_type", lang_code, "Invalid entity2_type.") # type: ignore
                    await interaction.followup.send(error_msg, ephemeral=True); return
            ...
            elif entity1_id is not None or e1_type_enum is not None: # If one is provided, both must be
                 error_msg = await get_localized_message_template(session, interaction.guild_id, "relationship_list:error_partial_e1_filter", lang_code, "If filtering by entity1, both ID and Type must be provided.") # type: ignore
                 await interaction.followup.send(error_msg, ephemeral=True); return
            ...
            elif entity2_id is not None or e2_type_enum is not None: # If one is provided, both must be
                 error_msg = await get_localized_message_template(session, interaction.guild_id, "relationship_list:error_partial_e2_filter", lang_code, "If filtering by entity2, both ID and Type must be provided.") # type: ignore
                 await interaction.followup.send(error_msg, ephemeral=True); return
            ...
            if len(embed.fields) == 0: # Should not happen if `not relationships` check passes
    ...
    async def relationship_create(self, interaction: discord.Interaction,
        ...
        async def validate_entity(session: Any, entity_id_val: int, entity_type_str_val: str, entity_type_enum_val: RelationshipEntityType, guild_id_val: int, entity_label: str) -> bool:
            ...
            if not crud_instance:
                error_msg_loc = await get_localized_message_template(session, guild_id_val, "relationship_create:error_unsupported_type", lang_code, f"Unsupported entity type for {entity_label}: {{type}}") # type: ignore
        ...
        async with get_db_session() as session:
            try:
                ...
                error_msg = await get_localized_message_template(session, interaction.guild_id, "relationship_create:error_invalid_entity_type", lang_code, "Invalid entity_type provided. Use PLAYER, GENERATED_NPC, or FACTION.") # type: ignore
                await interaction.followup.send(error_msg, ephemeral=True); return

            if not await validate_entity(session, entity1_id, entity1_type, e1_type_enum, interaction.guild_id, "Entity 1"): return # type: ignore
            if not await validate_entity(session, entity2_id, entity2_type, e2_type_enum, interaction.guild_id, "Entity 2"): return # type: ignore

            if entity1_id == entity2_id and e1_type_enum == e2_type_enum:
                error_msg = await get_localized_message_template(session, interaction.guild_id, "relationship_create:error_self_relationship", lang_code, "Entities cannot have a relationship with themselves.") # type: ignore
                await interaction.followup.send(error_msg, ephemeral=True); return

            existing_rel = await crud_relationship.get_relationship_between_entities(
                session, guild_id=interaction.guild_id, # type: ignore
                entity1_id=entity1_id, entity1_type=e1_type_enum,
                entity2_id=entity2_id, entity2_type=e2_type_enum
            )
            if existing_rel:
                error_msg = await get_localized_message_template(session, interaction.guild_id, "relationship_create:error_already_exists", lang_code, "A relationship between these entities already exists (ID: {id}). Use update instead.") # type: ignore
            ...
            try:
                ...
                error_msg = await get_localized_message_template(session, interaction.guild_id, "relationship_create:error_generic_create", lang_code, "Error creating relationship: {error}") # type: ignore
                await interaction.followup.send(error_msg.format(error=str(e)), ephemeral=True); return

            if not created_relationship:
                error_msg = await get_localized_message_template(session, interaction.guild_id, "relationship_create:error_creation_failed_unknown", lang_code, "Relationship creation failed.") # type: ignore
                await interaction.followup.send(error_msg, ephemeral=True); return
        ...
        async with get_db_session() as session:
            ...
            try:
                async with session.begin():
                    ...
                    await session.flush()
                    if updated_relationship: # Refresh only if not None
            ...
            if not updated_relationship: # Check after potential refresh
                error_msg = await get_localized_message_template(
                    session, interaction.guild_id, "relationship_update:error_update_failed_unknown", lang_code,
                    "Relationship update failed for an unknown reason."
                ) # type: ignore
