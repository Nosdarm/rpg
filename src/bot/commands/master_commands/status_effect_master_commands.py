...
from src.models.enums import StatusEffectCategory, RelationshipEntityType # RelationshipEntityType for ActiveSE
...
class MasterStatusEffectCog(commands.Cog, name="Master Status Effect Commands"):
    ...
    async def status_effect_def_view(self, interaction: discord.Interaction, status_effect_id: int):
        ...
        async with get_db_session() as session:
            ...
            if not se_def:
                ...
                if temp_se and temp_se.guild_id is None: se_def = temp_se
    ...
    async def status_effect_def_list(self, interaction: discord.Interaction, scope: Optional[app_commands.Choice[str]] = None, page: int = 1, limit: int = 10):
        ...
        async with get_db_session() as session:
            ...
            if scope_value == "guild": filters.append(status_effect_crud.model.guild_id == interaction.guild_id)
            elif scope_value == "global": filters.append(status_effect_crud.model.guild_id.is_(None))
            else: filters.append(or_(status_effect_crud.model.guild_id == interaction.guild_id, status_effect_crud.model.guild_id.is_(None)))
            offset = (page - 1) * limit
    ...
    async def status_effect_def_create(self, interaction: discord.Interaction, static_id: str, name_i18n_json: str, category: app_commands.Choice[str], description_i18n_json: Optional[str] = None, properties_json: Optional[str] = None, is_global: bool = False):
        ...
        async with get_db_session() as session:
            ...
            try:
                category_enum = StatusEffectCategory[category.value]
            ...
            embed.add_field(name="Static ID", value=created_se.static_id, inline=True); embed.add_field(name="Category", value=created_se.category.value, inline=True); embed.add_field(name="Scope", value=scope_disp, inline=True)
            await interaction.followup.send(embed=embed, ephemeral=True)
    ...
    async def status_effect_def_update(self, interaction: discord.Interaction, status_effect_id: int, field_to_update: app_commands.Choice[str], new_value: str):
        await interaction.response.defer(ephemeral=True)
        allowed_fields_map = {"static_id": {"db_field": "static_id", "type": str}, "name_i18n_json": {"db_field": "name_i18n", "type": dict}, "description_i18n_json": {"db_field": "description_i18n", "type": dict}, "category": {"db_field": "category", "type": StatusEffectCategory}, "properties_json": {"db_field": "properties_json", "type": dict},}
        lang_code = str(interaction.locale); command_field_name = field_to_update.value
        if command_field_name not in allowed_fields_map:
            async with get_db_session() as temp_session: error_msg = await get_localized_message_template(temp_session,interaction.guild_id,"se_def_update:error_field_not_allowed_internal",lang_code,"Internal error: Invalid field choice.")
            await interaction.followup.send(error_msg, ephemeral=True); return
        ...
        async with get_db_session() as session:
            ...
            if not se_def_to_update:
                temp_se = await status_effect_crud.get(session, id=status_effect_id)
                if temp_se and temp_se.guild_id is None: se_def_to_update = temp_se; original_guild_id = None
                else:
    ...
    async def status_effect_def_delete(self, interaction: discord.Interaction, status_effect_id: int):
        ...
        async with get_db_session() as session:
            ...
            if not se_def_to_delete:
                temp_se = await status_effect_crud.get(session, id=status_effect_id)
                if temp_se and temp_se.guild_id is None: se_def_to_delete = temp_se; is_global_to_delete = True
            if not se_def_to_delete:
            ...
            if not is_global_to_delete: active_check_filters.append(active_status_effect_crud.model.guild_id == interaction.guild_id)
    ...
    async def active_status_effect_remove(self, interaction: discord.Interaction, active_status_effect_id: int):
        ...
        lang_code = str(interaction.locale)
        async with get_db_session() as session:
            active_se_to_delete = await active_status_effect_crud.get_by_id_and_guild(session, id=active_status_effect_id, guild_id=interaction.guild_id)
