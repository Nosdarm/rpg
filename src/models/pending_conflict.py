...
class PendingConflict(Base, AsyncAttrs):
    ...
    involved_entities_json: Mapped[dict] = mapped_column(JSONB, nullable=False, comment="List of involved entity identifiers, e.g., [{'type': 'player', 'id': 123, 'name': 'PlayerName'}]")
    conflicting_actions_json: Mapped[dict] = mapped_column(JSONB, nullable=False, comment="List of ParsedAction objects (as dicts) that are in conflict")
    ...
    resolved_action_json: Mapped[dict] = mapped_column(JSONB, nullable=True, comment="If Master edits/chooses an action, it's stored here as a ParsedAction dict")
