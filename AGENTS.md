...
---
## Текущий план
*(Этот раздел будет очищен после завершения задачи)*
---
## Лог действий
...
- **Выполненные действия**:
    ...
    - Сгенерированы подробные описания для Task 0.1 (Discord Bot Project Initialization) и Task 0.2 (DBMS Setup and Database Model Definition).
    - Обновлен `AGENTS.md`:
        - Добавлена секция "требует доработки".
        - В секцию "требует доработки" добавлены ссылки на Task 0.1 и Task 0.2.
        - Текущая запись добавлена в "Лог действий".
- **Статус**: Подготовлены детальные описания для первых двух задач. `AGENTS.md` обновлен для отражения этой работы. Следующий шаг - представить эти детальные описания пользователю.
...
---
## требует доработки
*(Этот раздел для задач, описания которых в Tasks.txt были обновлены)*
...
## Документация API для UI Task 59: Генерация и Модерация AI контента

Эта документация описывает мастер-команды Discord, которые UI будет вызывать (концептуально, через API шлюз) для управления генерацией AI контента и его модерацией.

**Общие замечания:**

*   Все команды требуют `guild_id`, который UI должен передавать (обычно получается из контекста активной сессии UI).
*   Для полей JSON (`generation_context_json`, `new_parsed_data_json`) команды ожидают валидную JSON-строку.
*   Ответы команд будут содержать локализованные строки на языке Мастера.
*   Используемые TypeScript интерфейсы: `UIPendingGeneration`, `UIMRModerationStatus`, `TriggerGenerationPayload`, `UpdatePendingGenerationPayload` из `src/ui/src/types/pending_generation.ts` и `PaginatedResponse` из `src/ui/src/types/entities.ts`.

---

**1. Триггер генерации AI контента (`/master_pending_generation trigger`)**

*   **Команда Discord**: `/master_pending_generation trigger`
*   **Описание**: Инициирует процесс генерации AI контента, который затем попадает на модерацию.
*   **Параметры UI (`TriggerGenerationPayload`) -> Команда**:
    *   `entity_type: str` (Тип сущности для генерации, например, "location", "npc", "item", "quest", "faction"). **Важно**: Текущая реализация `ai_orchestrator.prepare_ai_prompt` может не полностью поддерживать все типы или требовать специфичного `generation_context_json`.
    *   `generation_context_json: Optional[str]` (JSON-строка для специфического контекста генерации, например, `{"theme": "forest", "difficulty": "hard"}`). Содержимое этого JSON сильно зависит от `entity_type` и логики в `prepare_ai_prompt`.
    *   `location_id_context: Optional[int]` (ID локации для контекста генерации, если применимо).
    *   `player_id_context: Optional[int]` (ID игрока для контекста генерации, если применимо).
*   **Ответ**: Сообщение об успехе с ID созданной записи `PendingGeneration` и ее статусом, или сообщение об ошибке.
    *   Пример успешного ответа (концептуальный, UI получит объект или сообщение):
        ```
        AI Generation Triggered
        Content generation has been queued for moderation.
        Pending ID: 123
        Status: PENDING_MODERATION
        Requested Type: npc
        Provided Context JSON: {"theme": "forest"}
        ```
*   **Замечание**: Эффективность и гибкость этой команды сильно зависят от реализации `ai_orchestrator.trigger_ai_generation_flow` и особенно `ai_prompt_builder.prepare_ai_prompt`, которые должны быть адаптированы для корректной обработки `entity_type` и `generation_context_json`.

---

**2. Список ожидающих модерации генераций (`/master_pending_generation list`)**

*   **Команда Discord**: `/master_pending_generation list`
*   **Описание**: Возвращает список записей `PendingGeneration` с фильтрацией и пагинацией.
*   **Параметры UI -> Команда**:
    *   `status_filter: Optional[str]` (Строковое значение из `UIMRModerationStatus` enum, например, "PENDING_MODERATION").
    *   `page: Optional[int]` (по умолчанию 1).
    *   `limit: Optional[int]` (по умолчанию 10, максимум 25).
*   **Ответ**: `PaginatedResponse<UIPendingGeneration>` (где `UIPendingGeneration` соответствует полям модели `PendingGeneration`).
    *   Каждый элемент списка будет содержать как минимум: `id`, `status`, `created_at`, `trigger_context_json` (из которого можно извлечь `requested_entity_type`).

---

**3. Просмотр деталей одной генерации (`/master_pending_generation view`)**

*   **Команда Discord**: `/master_pending_generation view`
*   **Описание**: Возвращает полную информацию о конкретной записи `PendingGeneration`.
*   **Параметры UI -> Команда**:
    *   `pending_generation_id: int` (ID записи).
*   **Ответ**: `UIPendingGeneration` (полная структура, включая `ai_prompt_text`, `raw_ai_response_text`, `parsed_validated_data_json`, `validation_issues_json`, `master_notes`).

---

**4. Одобрение генерации (`/master_pending_generation approve`)**

*   **Команда Discord**: `/master_pending_generation approve`
*   **Описание**: Одобряет генерацию и запускает процесс сохранения контента.
*   **Параметры UI -> Команда**:
    *   `pending_generation_id: int` (ID записи).
*   **Ответ**: Сообщение об успехе или ошибке. Статус записи `PendingGeneration` обновляется на `APPROVED` и затем `SAVED` (или `ERROR_ON_SAVE`).
*   **Условия**: Запись должна быть в статусе `PENDING_MODERATION`, `EDITED_PENDING_APPROVAL` или `VALIDATION_FAILED` (если Мастер решил одобрить несмотря на ошибки валидации, при условии что `parsed_validated_data_json` доступно или было исправлено через `update`).

---

**5. Обновление генерации (`/master_pending_generation update`)**

*   **Команда Discord**: `/master_pending_generation update`
*   **Описание**: Позволяет Мастеру изменить статус, заметки или отредактированные данные для `PendingGeneration`.
*   **Параметры UI (`UpdatePendingGenerationPayload`) -> Команда**:
    *   `pending_generation_id: int` (ID записи).
    *   `new_status: Optional[str]` (Новый статус из `UIMRModerationStatus`, например, "REJECTED", "EDITED_PENDING_APPROVAL"). Нельзя напрямую установить "APPROVED" или "SAVED" этой командой.
    *   `new_parsed_data_json: Optional[str]` (JSON-строка с отредактированными `parsed_validated_data_json`). Если данные изменяются, статус обычно должен переходить в `EDITED_PENDING_APPROVAL`.
    *   `master_notes: Optional[str]` (Заметки Мастера).
*   **Ответ**: Сообщение об успехе с обновленным статусом записи или сообщение об ошибке.

---

## Документация API для UI Task 67: Получение списка команд бота

Эта документация описывает API эндпоинт, который UI может использовать для получения списка доступных команд Discord-бота.

**Общие замечания:**

*   Все ответы API возвращаются в формате JSON.
*   `GUILD_ID_PLACEHOLDER` в примерах вызовов должен быть заменен на реальный ID гильдии, если это применимо для контекста UI, хотя сам эндпоинт списка команд не требует `guild_id` как параметр пути или запроса (но язык может зависеть от гильдии в будущем).

---

**1. Получить список всех команд бота**

*   **Эндпоинт**: `GET /api/v1/command-list/`
*   **Описание**: Возвращает список всех зарегистрированных слеш-команд бота с их описаниями и параметрами.
*   **Параметры запроса (Query Parameters)**:
    *   `language: Optional[str]` - Необязательный. Код языка (например, "en", "ru") для получения локализованных описаний команд и параметров, если они доступны. Если не указан, используются описания по умолчанию или на основном языке бота.
*   **Ответ (`200 OK`)**: `CommandListResponse` (см. `src/models/command_info.py`)
    *   **Структура `CommandListResponse`**:
        ```json
        {
          "commands": [
            {
              ...
              "parameters": [
                {
                  "name": "string",
                  "description": "string | null",
                  "type": "string (e.g., 'string', 'integer', 'user')",
                  "required": "boolean"
                  // "choices": [...] // TODO: Будет добавлено, если get_bot_commands начнет их извлекать
                }
              ]
              // "permissions": [...] // TODO: Будет добавлено, если get_bot_commands начнет их извлекать
            }
          ],
          "language_code": "string | null (код языка, который был использован для ответа)"
        }
        ```
*   **Пример вызова (curl)**:
    ```bash
    curl -X GET "http://localhost:8000/api/v1/command-list/?language=ru" -H "accept: application/json"
    ```
*   **Возможные ошибки**:
    *   `503 Service Unavailable`: Если экземпляр бота недоступен на сервере.
        ```json
        {
          "detail": "Bot instance not available. Cannot fetch commands."
        }
        ```
    *   `500 Internal Server Error`: В случае других внутренних ошибок при обработке запроса.
        ```json
        {
          "detail": "An internal error occurred while fetching commands: <сообщение об ошибке>"
        }
        ```
