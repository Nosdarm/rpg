...
- **План**: (см. "Текущий план" ниже)
- **Выполненные действия**:
    - **Шаг 1: Подготовка и Общие Компоненты**:
        - Создан файл `src/bot/commands/master_commands/master_simulation_tools_cog.py`.
        - В нем определен Cog `MasterSimulationToolsCog` с группами команд `/master_simulate` и `/master_analyze`.
        - Новый Cog добавлен в `BOT_COGS` в `src/config/settings.py`.
- **Статус**: Шаг 1 выполнен. Базовая структура для новых команд создана.
