...
1.  ***Детальный анализ `action_processor.py`***:
    *   Изучить `src/core/action_processor.py` для понимания текущей логики определения конфликтов, если она уже частично реализована для реальной игры.
    *   Особое внимание уделить функциям или концептуальным аналогам, связанным со сбором действий, группировкой конфликтующих действий, классификацией конфликтов и созданием `PendingConflict`.
2.  ***Проектирование "Dry Run" механизма для `action_processor.py`***:
    *   Разработать стратегию для вызова или адаптации логики определения конфликтов из `action_processor.py` таким образом, чтобы она не производила изменений в БД (например, добавление флага `dry_run` или рефакторинг для выделения "чистых" функций).
3.  ***Реализация в `simulate_conflict_detection`***:
    *   Модифицировать функцию `simulate_conflict_detection` в `src/core/conflict_simulation_system.py`.
    *   Заменить текущую упрощенную логику на вызовы адаптированных "dry run" функций из `action_processor.py` или воспроизвести его более точную логику.
    *   Обеспечить корректную передачу данных о симулируемых действиях (из команды `/master_simulate conflict`) в эту новую логику.
    *   Функция должна возвращать список Pydantic-моделей, аналогичных `PendingConflict`, но не сохраненных в БД.
4.  ***Расширение типов обнаруживаемых конфликтов***:
    *   Обеспечить поддержку более сложных сценариев конфликтов, которые может обрабатывать или должна будет обрабатывать `action_processor.py`. Это может включать конфликты не только по типу "один и тот же интент на одну и ту же цель", но и другие типы, определенные в `CONFLICT_RULES_SAME_INTENT_SAME_TARGET` и `CONFLICTING_INTENT_PAIRS_ON_SAME_TARGET` или новые.
5.  ***Учет `RuleConfig`***:
    *   Если `action_processor.py` использует `RuleConfig` для определения конфликтов, симуляция также должна это учитывать.
6.  ***Обновление Unit-тестов***:
    *   Написать или обновить Unit-тесты в `tests/core/test_conflict_simulation_system.py` для новой, более глубокой логики `simulate_conflict_detection`.
    *   Убедиться, что тесты покрывают различные сценарии конфликтов и использование "dry run" механизма.
7.  ***Обновление команды Discord***:
    *   При необходимости обновить команду `/master_simulate conflict` в `src/bot/commands/master_commands/master_simulation_tools_cog.py` для корректной работы с обновленной функцией `simulate_conflict_detection` и ее возвращаемыми данными.
8.  ***Обновление `AGENTS.md`***:
    *   Записать все предпринятые действия в "Лог действий" под заголовком "Task 48".
    *   Обновить секцию "Текущий план" в `AGENTS.md`.
    *   Если задача полностью завершена, очистить "Текущий план" и обновить статус задачи в логе. Если нет, обновить описание отложенной части.
9.  ***Завершение задачи (если применимо)***:
    *   Удалить Task 48 из `Tasks.txt`.
    *   Добавить Task 48 в `Done.txt`.
    *   Сделать коммит.
---
## Лог действий
