...
- **Оставшиеся доработки и ограничения (часть вынесена в отложенные задачи)**:
    - **Углубленная симуляция конфликтов**: Требует значительной доработки `simulate_conflict_detection` для тесной интеграции с логикой `action_processor.py` и поддержки "dry run". (См. Отложенную задачу: "Углубленная реализация системы симуляции конфликтов").
    - **Реальный вызов OpenAI в анализаторе AI**: Необходимо реализовать фактические вызовы к OpenAI API в `analyze_generated_content` при `use_real_ai=True`.
    - **Расширенная логика анализа в анализаторе AI**: Требуется определение большего количества правил в `RuleConfig` и реализация соответствующих проверок в `analyze_generated_content` для более глубокого анализа баланса, качества и соответствия лору сгенерированных сущностей.
    - **"Dry run" для симуляции боевых действий**: Команда `/master_simulate combat_action` на данный момент изменяет состояние БД. Для истинного "dry run" потребуются модификации в `combat_engine.py` и `game_events.py`. (Низкий приоритет для Task 48).
- **Обновление `AGENTS.md` от [YYYY-MM-DD]**: Добавлена отложенная задача по симуляции конфликтов. Обновлен лог Task 48.
...
- **Углубленная реализация системы симуляции конфликтов (`simulate_conflict_detection`)**:
    - **Описание**: Текущая реализация `simulate_conflict_detection` в `src/core/conflict_simulation_system.py` (для команды `/master_simulate conflict` в рамках Task 48) является упрощенной. Она определяет конфликты только на основе простого совпадения "эксклюзивных" интентов на одну и ту же сигнатуру цели. Для более точной и полезной симуляции требуется глубокая интеграция с логикой модуля `action_processor.py` с поддержкой "dry run".
    - **Необходимые действия**:
        1.  **Детальный анализ `action_processor.py`**:
            *   Тщательно изучить внутренние механизмы `action_processor.py`, отвечающие за определение конфликтующих групп действий (если таковые будут полностью реализованы в `action_processor.py` для реальной игры). Особое внимание уделить функциям (или их концептуальным аналогам, если они еще не выделены):
                *   Логика сбора действий, аналогичная `_get_relevant_entities_with_actions`.
                *   Логика группировки конфликтующих действий, аналогичная `_get_conflicting_actions_groups`.
                *   Логика классификации конфликтов, аналогичная `_determine_conflict_type_and_targets`.
                *   Структура данных `PendingConflict`, как в `_create_pending_conflicts_for_group`.
        2.  **Проектирование "Dry Run" механизма**:
            *   Разработать способ вызова или адаптации вышеуказанной логики таким образом, чтобы она не производила никаких изменений в базе данных.
            *   Это может потребовать рефакторинга `action_processor.py` для выделения "чистых" функций или добавления флага `dry_run`.
        3.  **Реализация в `simulate_conflict_detection`**:
            *   Заменить текущую упрощенную логику на вызовы адаптированных/новых "dry run" функций.
            *   Обеспечить корректную передачу данных о симулируемых действиях (`SimulatedActionActor`) в эту логику.
            *   Функция должна возвращать список Pydantic-моделей `PendingConflict` (не сохраненных в БД).
        4.  **Расширение типов обнаруживаемых конфликтов**:
            *   Поддержка более сложных сценариев конфликтов, которые может обрабатывать `action_processor.py`.
        5.  **Учет `RuleConfig`**: Если `action_processor.py` использует `RuleConfig` для определения конфликтов, симуляция также должна это учитывать.
        6.  **Обновление Unit-тестов**: Написать исчерпывающие тесты для новой логики `simulate_conflict_detection`.
    - **Срок**: Выполнить после стабилизации и полного внедрения механизма создания конфликтов в `action_processor.py`. Приоритет: высокий (для инструментов Мастера).
