...
---
## Текущий план
1.  ***Проектирование UX/UI для аутентификации и выбора гильдии.***
    *   Описать пользовательский сценарий (логин, редиректы Discord OAuth, выбор гильдии, сохранение сессии).
    *   Определить внешний вид и функциональность страниц/компонентов: LoginPage, AuthCallbackPage, GuildSelectionPage (если нужна), отображение информации о пользователе и гильдии в Layout.

2.  ***Уточнение деталей настройки Discord OAuth2 (теоретически).***
    *   Зафиксировать необходимые Redirect URIs (например, `http://localhost:PORT/auth/discord/callback`).
    *   Определить, какие OAuth scopes нужны (минимум `identify` и `guilds`).
    *   *Примечание: Предполагается, что Client ID будет доступен. Client Secret должен обрабатываться бэкендом, если не используется PKCE с прямым получением токена клиентом.*

3.  ***Реализация клиентской части Discord OAuth2 (описание компонентов и логики).***
    *   **LoginPage.tsx**: Добавить кнопку "Login with Discord". Логика кнопки: генерация URL для Discord OAuth (включая `client_id`, `redirect_uri`, `response_type=code`, `scope`, `state`, `code_challenge` для PKCE) и редирект пользователя.
    *   **AuthCallbackPage.tsx (`/auth/discord/callback`)**:
        *   Получение `code` и `state` из URL.
        *   Проверка `state` для предотвращения CSRF.
        *   Отправка `code` (и `code_verifier` для PKCE) на бэкенд для обмена на токен доступа ИЛИ (если бэкенд не участвует в обмене) прямой обмен на токен с Discord API (менее предпочтительно для безопасности Client Secret, но возможно с PKCE).
        *   *Предпочтительный сценарий: Бэкенд обрабатывает обмен кода на токен и возвращает сессионный токен/данные пользователя.*
    *   **AuthService.ts (или хук `useAuth`)**: Инкапсуляция логики аутентификации, включая формирование URL, обработку коллбэка, взаимодействие с хранилищем состояния и `localStorage`.

4.  ***Проектирование взаимодействия с бэкендом для аутентификации (если бэкенд-часть для OAuth не готова).***
    *   Описать API эндпоинт на бэкенде (например, `/api/auth/discord/finalize` или `/api/auth/session`), который принимает авторизационный код Discord, обменивает его на токен доступа Discord, получает данные пользователя от Discord, создает/обновляет сессию пользователя в бэкенде и возвращает UI необходимые данные (например, JWT сессии, информацию о пользователе, список доступных гильдий).
    *   Если бэкенд не будет иметь такого эндпоинта к моменту реализации UI, то UI будет получать токен Discord напрямую (используя PKCE) и затем запрашивать у бэкенда список "Мастер-гильдий" на основе этого токена.

5.  ***Управление состоянием аутентификации и сессией в UI (Zustand/Redux Toolkit).***
    *   **Хранилище состояния (например, `authStore` в Zustand)**:
        *   Поля: `isAuthenticated` (boolean), `user` (объект с данными пользователя Discord), `accessToken` (токен сессии от бэкенда или токен Discord), `selectedGuildId` (string | null), `availableGuilds` (GuildInfo[]), `status` ('idle', 'loading', 'error', 'success').
        *   Actions: `login(token, userData, guilds)`, `logout()`, `selectGuild(guildId)`, `setLoading()`, `setError()`, `clearError()`, `initializeAuth()`.
    *   **Сохранение токена**: Использование `localStorage` для персистентности сессионного токена. При инициализации приложения проверять наличие токена в `localStorage` и восстанавливать сессию.
    *   **Обновление HTTP-клиента (Axios)**: Интерцептор для автоматического добавления токена авторизации в заголовки запросов к бэкенду.

6.  ***Реализация выбора гильдии (описание компонентов и логики).***
    *   **Получение списка гильдий**: После успешной аутентификации, если `selectedGuildId` еще не установлен, запросить у бэкенда список гильдий, где пользователь является Мастером и бот активен (например, эндпоинт `/api/users/me/master-guilds`). Бэкенд для этого будет использовать токен сессии пользователя и, возможно, делать запросы к Discord API (`/users/@me/guilds`) и проверять свои данные.
    *   **GuildSelectionPage.tsx (или модальное окно)**:
        *   Если получен список из нескольких гильдий, отобразить их для выбора.
        *   При выборе гильдии, сохранить `selectedGuildId` в хранилище состояния и `localStorage`.
        *   Перенаправить пользователя на дашборд выбранной гильдии (например, `/guild/:guildId/dashboard`).
        *   Если доступна только одна гильдия, выбрать ее автоматически.

7.  ***Создание/доработка базовой структуры UI (макеты, навигация - описание компонентов).***
    *   **MainLayout.tsx**:
        *   Navbar: Отображение имени пользователя (из `authStore.user`), кнопки "Выйти" (вызывает `authStore.logout()` и редирект на `/login`). Отображение текущей выбранной гильдии (`authStore.selectedGuildId`) и, возможно, кнопки/меню для смены гильдии (очищает `selectedGuildId` и перенаправляет на страницу выбора).
        *   Sidebar: Навигационные ссылки (пока заглушки) на основные разделы (`/guild/:guildId/dashboard`, `/guild/:guildId/players` и т.д.). Ссылки должны быть активны только если гильдия выбрана.
    *   **ProtectedRoute.tsx (или `AuthGuard`)**: Компонент-обертка для маршрутов.
        *   Проверяет `authStore.isAuthenticated`. Если нет, редирект на `/login`.
        *   Проверяет `authStore.selectedGuildId` для маршрутов внутри гильдии. Если нет, редирект на страницу выбора гильдии (или `/login`, если это более уместно).
    *   **AppRouter.tsx**: Обновить конфигурацию маршрутов для использования `ProtectedRoute`.

8.  ***Настройка Axios для автоматической передачи `guild_id` (описание).***
    *   Создать интерцептор запросов в Axios, который будет проверять наличие `selectedGuildId` в `authStore`.
    *   Если `selectedGuildId` есть и запрос идет к API бэкенда (не к Discord), добавлять `X-Guild-ID: selectedGuildId` в заголовки запроса.

9.  ***Обновление `AGENTS.md` (лог и текущий план).***
    *   Записать этот план в "Текущий план".
    *   По мере (теоретического) выполнения шагов, обновлять "Лог действий".

10. ***Завершение задачи (обновление `Tasks.txt`, `done.txt`, коммит).***
    *   После описания всех шагов, обновить файлы задач.
    *   Подготовить коммит с описанием теоретической реализации Task 56.
---
## Лог действий

## Временный план: Создание базового каркаса UI-проекта (для Task 55-57)
- **Дата**: 2024-07-30
- **Цель**: Создать файловую структуру и базовые конфигурационные файлы для нового UI-приложения на React/Vite.
- **Выполненные действия**:
    - **Шаг 1: Создан `ui/package.json`**: Файл создан с перечислением основных зависимостей (React, Vite, TypeScript, React Router, Zustand, TanStack Query, Axios, MUI, i18next, react-hook-form) и dev-зависимостей. Пользователю необходимо будет выполнить `npm install` (или аналог) в директории `ui/`.
    - **Шаг 2: Создан `ui/vite.config.ts`**: Файл создан с базовой конфигурацией Vite, включая React плагин, алиас для `src/`, настройки сервера разработки и директории сборки.
    - **Шаг 3: Созданы `ui/tsconfig.json` и `ui/tsconfig.node.json`**: Файлы созданы с базовой конфигурацией TypeScript для React-проекта, совместимой с Vite, включая строгие проверки, JSX transform, алиасы путей и `noEmit`.
    - **Шаг 4: Создан `ui/index.html`**: Файл создан как точка входа для Vite-приложения, с базовой HTML-структурой, мета-тегами, ссылками на шрифты Material UI и точкой монтирования React-приложения.
    - **Шаг 5: Создана базовая структура директорий в `ui/src/`**: Созданы директории `app`, `assets`, `components` (с `common`, `layout`, `auth`), `features`, `hooks`, `lib`, `pages`, `providers`, `routes`, `services`, `styles`, `translations` (с `en`, `ru`), `types` с файлами `.gitkeep`.
    - **Шаг 6: Созданы ключевые файлы-заглушки в `ui/src/`**:
        - `index.tsx` (точка входа React с настройкой провайдеров)
        - `vite-env.d.ts` (типы для Vite env)
        - `app/App.tsx` (корневой компонент приложения с инициализацией auth)
        - `app/router.tsx` (базовый роутинг с lazy loading страниц)
        - `app/stores/authStore.ts` (хранилище Zustand для аутентификации)
        - `components/layout/MainLayout.tsx` (компонент основного макета)
        - `components/layout/Navbar.tsx` (компонент верхней панели)
        - `components/layout/Sidebar.tsx` (компонент боковой панели)
        - `pages/LoginPage.tsx` (компонент страницы входа)
        - `pages/AuthCallbackPage.tsx` (компонент обработки OAuth callback)
        - `pages/GuildSelectionPage.tsx` (компонент выбора гильдии)
        - `pages/DashboardPage.tsx` (компонент дашборда)
        - `routes/ProtectedRoute.tsx` (компонент защиты маршрутов)
        - `lib/apiClient.ts` (настроенный экземпляр Axios)
        - `lib/i18n.ts` (настройка i18next с базовыми ресурсами)
        - `lib/pkceUtils.ts` (утилиты для PKCE)
        - `styles/global.css` (пустой файл для глобальных стилей)
- **Статус**: Основной каркас UI-проекта создан. Файлы содержат базовую структуру и заглушки. Дальнейшая детализация и реализация бизнес-логики будет производиться в рамках следующих UI-задач (Task 57 и далее).

## Task 56: 🖥️ UI.2 Basic UI Structure and Authentication Development
- **Дата**: 2024-07-30
- **Определение задачи**: Create the main structure of the UI application (navigation, page layouts). Implement a UI user authentication system (e.g., via Discord OAuth2). Implement selection of the guild. UI must store Guild ID and pass it in requests.
- **Выполненные действия (теоретическое описание, без создания файлов):**
    - **Шаг 1: Проектирование UX/UI для аутентификации и выбора гильдии**: Описан пользовательский сценарий входа, редиректов Discord OAuth, выбора гильдии и сохранения сессии. Определены ключевые страницы/компоненты (`LoginPage`, `AuthCallbackPage`, `GuildSelectionPage`, `MainLayout`) и их UX.
    - **Шаг 2: Уточнение деталей настройки Discord OAuth2**: Зафиксированы необходимые Redirect URIs, OAuth scopes (`identify`, `guilds`), использование Client ID, стратегия обработки Client Secret (бэкендом), рекомендован Authorization Code Grant с PKCE, и меры безопасности (state-параметр).
    - **Шаг 3: Реализация клиентской части Discord OAuth2**: Описаны компоненты `LoginPage.tsx` (инициация OAuth), `AuthCallbackPage.tsx` (обработка ответа Discord, обмен кода на токен через бэкенд или напрямую с PKCE), `AuthService.ts` (инкапсуляция логики). Приведен пример утилиты для PKCE и переменных окружения.
    - **Шаг 4: Проектирование взаимодействия с бэкендом для аутентификации**: Описаны API эндпоинты бэкенда (`/api/auth/discord/finalize`, `/api/auth/session/me`, `/api/auth/logout`), их запросы и ответы. Основной сценарий: бэкенд обменивает код Discord на токен и управляет сессиями UI. Рассмотрен альтернативный сценарий.
    - **Шаг 5: Управление состоянием аутентификации и сессией в UI (Zustand)**: Определена структура хранилища `AuthState` в Zustand (user, accessToken, selectedGuildId, availableGuilds, status, error, actions). Использование `persist` middleware для `localStorage`. Описана инициализация состояния и обновление Axios интерцепторов для автоматического добавления токена.
    - **Шаг 6: Реализация выбора гильдии**: Описана логика получения списка "Мастер-гильдий" от бэкенда, компонент `GuildSelectionPage.tsx` для выбора, механизм автоматического выбора одной гильдии, и сохранение `selectedGuildId` в `localStorage` через `authStore`.
    - **Шаг 7: Создание/доработка базовой структуры UI (макеты, навигация)**: Описаны компоненты `MainLayout.tsx` (основной макет), `Navbar.tsx` (верхняя панель), `Sidebar.tsx` (боковая навигация), и `ProtectedRoute.tsx` (защита маршрутов).
    - **Шаг 8: Настройка Axios для автоматической передачи `guild_id`**: Описана доработка интерцептора запросов Axios для добавления заголовка `X-Guild-ID` к запросам на API бэкенда, если гильдия выбрана.
- **Статус**: Все теоретические аспекты проектирования и описания компонентов/логики для Task 56 завершены.
...
- **Выполненные действия**:
    - **Шаг 1: Исследование и анализ вариантов UI-технологий**: Проведен анализ UI-фреймворков React, Vue и Angular на основе общих знаний. React и Vue выделены как наиболее подходящие.
    - **Шаг 2: Предложение технологического стека и запрос решения у пользователя**: Пользователю предложен выбор между React и Vue. **Пользователь выбрал React.**
    - **Шаг 3: Определение базовой архитектуры UI-приложения**:
        - **Основной фреймворк**: React.
        - **Сборщик/CLI**: Vite.
        - **Язык**: TypeScript.
        - **Роутинг**: React Router (`react-router-dom`).
        - **Управление состоянием (Клиент)**: Zustand (предпочтительно для начала) или Redux Toolkit.
        - **Управление состоянием (Сервер)**: TanStack Query (React Query).
        - **HTTP-клиент**: Axios.
        - **UI-компоненты**: Material UI (MUI) (предложено как стартовый вариант).
        - **Интернационализация (i18n)**: react-i18next.
        - **Формы**: React Hook Form.
        - Предложена структура директорий (см. предыдущие логи агента).
        - Определены примерные базовые маршруты.
    - **Шаг 4: Создание базового каркаса UI-проекта**: Этот шаг выполнен теоретически, так как агент не может напрямую создавать файлы проекта. Описаны команды для инициализации проекта с Vite (React + TS), установки зависимостей, а также приведены примеры кода для ключевых файлов (`index.tsx`, `App.tsx`, `router.tsx`, `MainLayout.tsx`, `DashboardPage.tsx`, `LoginPage.tsx`) и концептуальной настройки Zustand, TanStack Query, Axios, i18next.
- **Статус**: Основные решения по стеку и архитектуре приняты. Каркас проекта описан теоретически.
