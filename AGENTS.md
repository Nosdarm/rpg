...
Задача "🧠 2.1 Finalize Definition of ALL DB Schemas (i18n, Guild ID)" почти завершена. Осталось записать ее в `Done.txt` и удалить из `Tasks.txt`.

Следующая задача согласно `Tasks.txt` (после завершения 2.1): "🧠 2.2 AI Prompt Preparation Module."

1.  **Завершение Задачи 2.1**:
    *   Записать "Task 2.1 Finalize Definition of ALL DB Schemas (i18n, Guild ID)" в `Done.txt`.
    *   Удалить Task 2.1 из `Tasks.txt`.
2.  **Анализ требований Задачи 2.2 (AI Prompt Preparation Module)**:
    *   API: `prepare_ai_prompt(guild_id: int, location_id: int, player_id: Optional[int] = None, party_id: Optional[int] = None) -> str`.
    *   Сбор контекста мира для указанной гильдии: WorldState (36), relationships (30/31/32), quests (35/36), lore (4), party level (5).
    *   Включение "словаря" игровых терминов (статы, скиллы, сущности) из БД для данной гильдии (14/13/30/36/37) как API для ИИ.
    *   Запрос на генерацию сущностей (NPC, Quests, Items, Locations, Events) в соответствии с правилами и контекстом гильдии.
    *   Генерация текстов на основном языке бота для гильдии (1) И на английском, в формате _i18n JSONB.
3.  **Планирование реализации Задачи 2.2**.
    *   Определить структуру модуля `ai_prompt_builder.py` в `src/core/`.
    *   Разработать функции для сбора каждого типа контекстной информации (world state, relationships, quests, lore, etc.), учитывая `guild_id`.
    *   Разработать механизм формирования "словаря" игровых терминов.
    *   Разработать логику объединения всей информации в единый промпт.
    *   Продумать обработку опциональных `player_id` и `party_id`.
4.  **Реализация Задачи 2.2**.
5.  **Обновить `AGENTS.md`**.
6.  **Записать выполненную задачу в `Done.txt`**.
7.  **Удалить выполненную задачу из `Tasks.txt`**.
