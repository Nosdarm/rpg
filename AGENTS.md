...
---
## Текущий план
1.  **Создать CRUD для модели `GeneratedFaction`**:
    *   Создать файл `src/core/crud/crud_faction.py`.
    *   Реализовать класс `CRUDFaction`, унаследованный от `CRUDBase[GeneratedFaction]`.
    *   Экспортировать экземпляр `crud_faction` из этого модуля.
    *   Добавить `crud_faction` в `src/core/crud/__init__.py`.

2.  **Расширить Pydantic-модели для парсинга ответа AI**:
    *   В `src/core/ai_response_parser.py`:
        *   Создать Pydantic-модель `ParsedFactionData(BaseGeneratedEntity)` для данных о фракциях. Она должна включать поля, соответствующие `GeneratedFaction` (например, `name_i18n`, `description_i18n`, `ideology_i18n`, `static_id` (опционально, AI может его предложить), `leader_npc_static_id` (опционально), `resources_json`, `ai_metadata_json`).
        *   Создать Pydantic-модель `ParsedRelationshipData(BaseGeneratedEntity)` для данных об отношениях. Она должна включать поля для определения двух сущностей (например, `entity1_static_id`, `entity1_type`, `entity2_static_id`, `entity2_type`), типа и значения отношения. `static_id` будет использоваться для связи с генерируемыми фракциями до того, как у них появятся ID из БД.
        *   Обновить `GeneratedEntity = Union[...]`, добавив `ParsedFactionData` и `ParsedRelationshipData`.
        *   Дополнить `_perform_semantic_validation` для проверки корректности данных в `ParsedFactionData` и `ParsedRelationshipData` (например, проверка обязательных полей, форматов i18n).

3.  **Разработать функцию для подготовки промпта AI для генерации фракций и отношений**:
    *   В `src/core/ai_prompt_builder.py`:
        *   Создать новую асинхронную функцию `async def prepare_faction_relationship_generation_prompt(session: AsyncSession, guild_id: int) -> str`.
        *   Эта функция должна:
            *   Загружать правила генерации фракций и отношений из `RuleConfig` для `guild_id` (например, количество фракций, типы, сложность отношений, тематика).
            *   Формировать системный промпт, объясняющий AI задачу: сгенерировать указанное количество фракций с их характеристиками и определить отношения между ними (и, возможно, с глобальными сущностями типа "игрок по умолчанию" или "мир").
            *   Включить в промпт схемы для `faction` и `relationship` (аналогично `_get_entity_schema_terms`), чтобы AI возвращал данные в ожидаемом формате. Указать, что `static_id` для фракций должен быть сгенерирован AI и быть уникальным в рамках ответа.
            *   Указать на необходимость локализации текстовых полей (`_i18n`).

4.  **Реализовать основную функцию для генерации фракций и отношений**:
    *   В `src/core/world_generation.py`:
        *   Создать новую асинхронную функцию `async def generate_factions_and_relationships(session: AsyncSession, guild_id: int, trigger_event_id: Optional[int] = None) -> Tuple[Optional[List[GeneratedFaction]], Optional[List[Relationship]], Optional[str]]`.
        *   Эта функция должна:
            *   Вызвать `prepare_faction_relationship_generation_prompt` для получения промпта.
            *   Вызвать AI (например, `_mock_openai_api_call` или реальный API call через `ai_orchestrator`).
            *   Вызвать `parse_and_validate_ai_response` для обработки ответа AI.
            *   Если парсинг успешен:
                *   Итерировать по `ParsedFactionData` из ответа:
                    *   Сохранить каждую фракцию в БД с помощью `crud_faction.create`, присвоив `guild_id`. Сохранить соответствие между временным `static_id` из AI-ответа и реальным `id` из БД.
                *   Итерировать по `ParsedRelationshipData` из ответа:
                    *   Используя сохраненное соответствие `static_id` -> `db_id`, преобразовать `entityX_static_id` в `entityX_id`.
                    *   Определить `entityX_type` (в данном случае, вероятно, `RelationshipEntityType.FACTION`).
                    *   Сохранить каждое отношение в БД с помощью `crud_relationship.create`, присвоив `guild_id`.
                *   Залогировать событие `FACTIONS_RELATIONSHIPS_GENERATED` с деталями.
            *   Вернуть списки созданных фракций и отношений или сообщение об ошибке.
    *   Экспортировать новую функцию из `src/core/__init__.py`.

5.  **Обновить `AGENTS.md`**:
    *   Записать текущий план в секцию "Текущий план".
    *   Начать лог действий для Task 35.

6.  **Написать Unit-тесты (концептуально, реализация после основного кода)**:
    *   Тесты для `crud_faction`.
    *   Тесты для новых Pydantic-моделей и их валидации в `ai_response_parser.py`.
    *   Тесты для `prepare_faction_relationship_generation_prompt`, проверяющие формирование промпта.
    *   Интеграционные тесты для `generate_factions_and_relationships`, мокируя вызов AI и проверяя сохранение данных в БД и логирование.

7.  **Завершение задачи**:
    *   После успешной реализации и прохождения тестов (если они будут написаны на этом этапе):
        *   Удалить задачу 35 из `Tasks.txt`.
        *   Добавить информацию о выполненной задаче 35 в `Done.txt`.
        *   Обновить "Лог действий" в `AGENTS.md`.
...
- **Реализация**:
    ...
    - **Шаг 6: Unit-тесты**: Концептуально запланированы, но не реализованы на данном этапе.
- **Статус**: Задача выполнена. Система генерации фракций и отношений базово реализована и готова к использованию (с мок-ответом от AI). Требуется добавление реального вызова LLM и написание Unit-тестов в будущем.
