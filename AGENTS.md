...
4.  **Написать базовые Unit-тесты**.
    *   Для `prepare_dialogue_generation_prompt`:
        *   Проверить, что функция собирает основной контекст (NPC, игрок, язык).
        *   Проверить, что промпт содержит ключевые инструкции для LLM.
        *   Проверить, что используются заглушки для NPC Memory.
    *   Для `generate_npc_dialogue`:
        *   Проверить, что функция вызывает `prepare_dialogue_generation_prompt` и мок LLM.
        *   Проверить, что возвращается ожидаемая строка из мок-ответа.
5.  **Заглушки и реальные CRUD**.
    *   В `ai_prompt_builder.py` заменить используемые `PlaceholderCRUDBase` для `generated_npc_crud` и `relationship_crud` на импорты реальных CRUD-операций из `src.core.crud` (по аналогии с `location_crud`, `player_crud`). Если реальных CRUD нет, создать минимальные версии в соответствующих файлах (`crud_npc.py`, `crud_relationship.py`), наследуя от `CRUDBase`.
    *   Убедиться, что `guild_config_crud` также является реальным CRUD. Если нет, создать `crud_guild.py`.
---
## Лог действий

## Task 50: 🧠 11.1 Dialogue Generation Module (LLM, Multy-i18n, According to Rules) - Сессия 2024-07-26
...
- **Анализ файлов**:
    - Проанализированы файлы `src/core/ai_prompt_builder.py`, `src/core/dialogue_system.py`, `src/core/__init__.py`, `tests/core/test_ai_prompt_builder.py`, `tests/core/test_dialogue_system.py` и связанные CRUD.
    - `prepare_dialogue_generation_prompt` в `ai_prompt_builder.py` уже детально реализована, собирает обширный контекст.
    - `generate_npc_dialogue` в `dialogue_system.py` вызывает `prepare_dialogue_generation_prompt` и использует МОК для LLM.
    - Экспорты в `__init__.py` корректны.
    - Существующие тесты покрывают базовые сценарии.
    - Используются реальные CRUD, а не заглушки.
- **Текущий план (скопирован из `AGENTS.md` от пользователя и немного доработан)**:
    1.  **Обновить `AGENTS.md`**: (Этот шаг)
        *   Зафиксировать текущий прогресс выполнения Task 50 в секции "Лог действий".
        *   Скопировать и немного доработать существующий "Текущий план" из `AGENTS.md` для этой сессии.
    2.  **Доработка Unit-тестов для `prepare_dialogue_generation_prompt` в `tests/core/test_ai_prompt_builder.py`**:
        *   Проверка сбора основного контекста (NPC, игрок, язык).
        *   Проверка ключевых инструкций для LLM.
        *   Проверка использования заглушки для NPC Memory.
        *   Проверка контекста Отношений.
        *   Проверка контекста Скрытых Отношений.
        *   Проверка контекста Квестов.
    3.  **Доработка Unit-тестов для `generate_npc_dialogue` в `tests/core/test_dialogue_system.py`**:
        *   Убедиться, что существующие тесты покрывают вызов `prepare_dialogue_generation_prompt` и мок LLM, а также возврат ожидаемой строки.
        *   Рассмотреть добавление теста для длинного/спецсимвольного `player_input_text`.
    4.  **Запуск всех тестов и исправление ошибок**.
    5.  **Обновление `AGENTS.md`** (лог действий, статус задачи, очистка плана).
    6.  **Коммит изменений**.
- **Статус на 2024-07-26**:
    - Шаги 1 и 2 первоначального плана (создание функций `prepare_dialogue_generation_prompt` и `generate_npc_dialogue`) были в основном выполнены до начала текущей сессии.
    - Шаг 5 первоначального плана (использование реальных CRUD) также был выполнен.
    - **Доработаны Unit-тесты для `prepare_dialogue_generation_prompt` (`tests/core/test_ai_prompt_builder.py`)**:
        - Расширены проверки в `test_prepare_dialogue_prompt_basic_success` для покрытия:
            - Сбора основного контекста NPC (имя, описание, роль, личность, стиль диалога).
            - Сбора основного контекста игрока (имя, уровень).
            - Указания языка в промпте.
            - Наличия ключевых инструкций для LLM.
            - Вызова и использования результата мока `_get_npc_memory_context_stub`.
        - Добавлен новый тест `test_prepare_dialogue_prompt_with_relationship_quest_hidden_context` для проверки:
            - Включения информации о существующих отношениях между игроком и NPC.
            - Включения информации из `_get_hidden_relationships_context_for_dialogue`.
            - Включения информации об активных квестах игрока из `_get_quests_context`.
        - Добавлен новый тест `test_prepare_dialogue_prompt_no_relationship` для проверки корректного сообщения при отсутствии установленных отношений.
    - **Доработаны Unit-тесты для `generate_npc_dialogue` (`tests/core/test_dialogue_system.py`)**:
        - Существующие тесты подтверждены как покрывающие основные требования (вызов `prepare_dialogue_generation_prompt`, мок LLM, возврат строки).
        - Добавлен тест `test_generate_npc_dialogue_with_special_player_input` для проверки обработки длинных строк и спецсимволов в `player_input_text`.
    - **Все тесты (773) успешно пройдены** после исправлений в `tests/core/test_ai_prompt_builder.py`.
- **Оставшиеся шаги по плану на текущую сессию**:
    - Обновление `AGENTS.md` (этот лог).
    - Очистка секции "Текущий план".
    - Коммит изменений.
