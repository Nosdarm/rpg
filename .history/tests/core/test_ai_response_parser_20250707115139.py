...
class TestAIResponseParserPydanticModels(unittest.TestCase):
    ...
    def test_parsed_npc_data_valid(self):
        ...
        valid_npc_data = {
            ...
            "stats": {"hp": 50} # Assuming level might be added by a default or is optional here
        }
...
>>>>>>> 3648882d7ce127ff9cdbdd88b7ec75d55362e395
    ...
    def test_parsed_npc_trader_data_invalid(self):
        with self.assertRaisesRegex(PydanticNativeValidationError, "must be a non-empty dictionary if provided"):
            ParsedNpcTraderData(entity_type="npc_trader", static_id="t1", name_i18n={"en":"n"}, description_i18n={"en":"d"}, role_i18n={}) # level is optional
...
class TestAIResponseParserFunction(unittest.IsolatedAsyncioTestCase):
    async def test_parse_valid_faction_and_relationship(self):
        ...
        # The parse_and_validate_ai_response function is expected to return ParsedAiData or CustomValidationError
        # If it returns ParsedAiData, raw_ai_output is part of that model.
        result = await parse_and_validate_ai_response(ai_response_str, guild_id=1)
        if isinstance(result, CustomValidationError):
            self.fail(f"Parsing failed: {result.message} - Details: {result.details}")
        self.assertIsInstance(result, ParsedAiData)
        self.assertEqual(result.raw_ai_output, ai_response_str) # Check raw_ai_output is present
    ...
    async def test_parse_valid_quest_data(self):
        ...
        # loop = asyncio.get_event_loop() # Not needed for async test methods
        result = await parse_and_validate_ai_response(valid_quest_json_str, guild_id=1)
        self.assertIsInstance(result, ParsedAiData)
        assert isinstance(result, ParsedAiData) # for type checker
        self.assertEqual(result.raw_ai_output, valid_quest_json_str)
    ...
    async def test_parse_quest_data_missing_static_id(self):
        ...
        # loop = asyncio.get_event_loop() # Not needed
        result = await parse_and_validate_ai_response(invalid_quest_json_str, guild_id=1)
        self.assertIsInstance(result, CustomValidationError)
    ...
    async def test_parse_quest_data_empty_steps(self):
        invalid_quest_json_str = """
        [
    ...
    async def test_parse_quest_step_data_invalid_i18n(self):
        ...
        # loop = asyncio.get_event_loop() # Not needed
        result = await parse_and_validate_ai_response(invalid_step_json_str, guild_id=1)
        self.assertIsInstance(result, CustomValidationError)
        assert isinstance(result, CustomValidationError) # for type checker
        self.assertEqual(result.error_type, "StructuralValidationError")
        assert result.details is not None # for type checker
    ...
    # This test_generated_inventory_item_entry_valid seems to be duplicated. Removing one instance.
    # def test_generated_inventory_item_entry_valid(self):
    #     valid_entry = {
    #         "item_static_id": "potion_heal", "quantity_min": 1, "quantity_max": 5, "chance_to_appear": 0.8
    #     }
    ...
    # This test_generated_inventory_item_entry_invalid seems to be duplicated. Removing one instance.
    # def test_generated_inventory_item_entry_invalid(self):
    #     with self.assertRaisesRegex(PydanticNativeValidationError, "item_static_id must be a non-empty string"):
    #         GeneratedInventoryItemEntry(item_static_id="", quantity_min=1, quantity_max=1, chance_to_appear=1.0)
    #     with self.assertRaisesRegex(PydanticNativeValidationError, "Quantity must be a positive integer"): # This message might change with Pydantic v2 Field(ge=1)
    #         GeneratedInventoryItemEntry(item_static_id="id", quantity_min=0)
    ...
    # This test_parsed_npc_trader_data_valid seems to be duplicated. Removing one instance.
    # def test_parsed_npc_trader_data_valid(self):
    #     valid_trader_data = {
    #         "entity_type": "npc_trader", "static_id": "trader_01",
    #         "name_i18n": {"en": "Trader Bob", "ru": "Торговец Боб"},
    ...
    # This test_parsed_npc_trader_data_invalid seems to be duplicated. Removing one instance.
    # def test_parsed_npc_trader_data_invalid(self):
    #     with self.assertRaisesRegex(PydanticNativeValidationError, "must be a non-empty dictionary if provided"):
    ...
    #             generated_inventory_items=[{"item_static_id": ""}]
    #         )
    ...
    async def test_parse_valid_item_and_trader_data(self):
        ai_response_list = [
            ...
            {
                "entity_type": "npc_trader", "static_id": "trader_01",
                "name_i18n": {"en": "Trader Bob"}, "description_i18n": {"en": "Sells goods."}, # level is optional
        ...
        self.assertEqual(result.raw_ai_output, ai_response_str)
        self.assertEqual(len(result.generated_entities), 2)
    ...
    async def test_parse_pydantic_validation_error_in_faction(self):
        ...
        del invalid_faction_data["name_i18n"] # This will cause a validation error
        ...
        if isinstance(result, CustomValidationError):
            ...
            if result.details:
                for error_detail in result.details:
                    # Pydantic v2 loc is a tuple of path elements
                    if isinstance(error_detail.get("loc"), tuple) and "name_i18n" in error_detail.get("loc", tuple()) and error_detail.get("type") == "missing":
                        found_name_error = True; break
            self.assertTrue(found_name_error, f"Details: {result.details}")
    ...
    async def test_parse_semantic_validation_error_faction_missing_lang(self):
        ...
        result = await parse_and_validate_ai_response(ai_response_str, guild_id=1) # raw_ai_output is handled by the func
        self.assertIsInstance(result, CustomValidationError)
        if isinstance(result, CustomValidationError):
    ...
    async def test_parse_semantic_validation_error_relationship_value_out_of_range(self):
        ...
        # Ensure all required i18n fields are present for the faction to pass its own validation
...
# Removed if __name__ == "__main__": block as pytest handles test execution
