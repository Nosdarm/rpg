...
from backend.config import settings # Импортируем наши настройки
from backend.core.database import get_db_session # CORRECTED IMPORT
from backend.core.crud import crud_master_user
from backend.schemas.master_user import MasterUserCreate # Pydantic схема для создания MasterUser
from backend.core.security import create_access_token, TokenPayload, get_current_token_payload # get_current_token_payload for Depends
...
async def discord_callback(code: str, session: AsyncSession = Depends(get_db_session)): # CORRECTED
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            from backend.models.guild import GuildConfig # Локальный импорт для проверки
...
async def get_my_guilds(
    ...
    from backend.core.crud.crud_guild import guild_crud # Используем существующий crud_guild
    from backend.models.guild import GuildConfig # Используем существующую модель GuildConfig
    # from backend.schemas.guild import GuildSchema # Если бы возвращали полную схему
...
# from backend.core.security import TokenPayload, get_current_token_payload
#
...
# и что `uvicorn` запускает именно этот `app` из `backend.main`.
# Пример src/main.py, если его нет:
"""
from fastapi import FastAPI, Depends # Добавлен Depends
from backend.api.routers import auth as auth_router # Пример
from backend.core.security import get_current_master_user, TokenPayload # Для тестового эндпоинта
from backend.models.master_user import MasterUser # Для тестового эндпоинта
from backend.schemas.master_user import MasterUserSchema # Для тестового эндпоинта
# from backend.core.database import init_db # Если есть функция инициализации БД
...
# Для запуска: uvicorn backend.main:app --reload --host 0.0.0.0 --port 8000
"""
...
from typing import Optional # Для response_model в get_active_guild
# from backend.core.security import get_current_token_payload # Уже импортирован выше
