...
# from backend.models.player import PlayerStatus # If needed for direct enum comparison
...
class MasterPlayerCog(commands.Cog, name="Master Player Commands"): # type: ignore[call-arg]
    ...
    async def player_create(
        ...
        async with get_db_session() as session:
            ...
            from backend.bot.utils import parse_json_parameter # Import utility
    ...
    async def player_update(self, interaction: discord.Interaction, player_id: int, field_to_update: str, new_value: str):
        ...
        from backend.models.enums import PlayerStatus # Ensure PlayerStatus is available
            ...
            try:
                async with session.begin_nested(): # Use nested transaction
                    ...
                    if party_id_of_player:
                        from backend.core.crud.crud_party import party_crud # Local import to avoid circular dependency at module level
