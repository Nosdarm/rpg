...
from backend.core.crud.crud_combat_encounter import combat_encounter_crud
# from backend.core.crud.crud_player import player_crud # Potentially for resetting status on delete
from backend.core.database import get_db_session
from backend.core.crud_base_definitions import update_entity # Now used
from backend.core.localization_utils import get_localized_message_template
from backend.models.enums import CombatStatus # For status validation
from backend.bot.utils import parse_json_parameter # Import the utility
# from backend.models.enums import PlayerStatus # If resetting status on delete

logger = logging.getLogger(__name__)

class MasterCombatEncounterCog(commands.Cog, name="Master Combat Encounter Commands"): # type: ignore[call-arg]
    ...
    async def combat_encounter_view(self, interaction: discord.Interaction, encounter_id: int):
        ...
        async with get_db_session() as session:
            ...
            embed.add_field(name=await get_label("status", "Status"), value=encounter.status.value if encounter.status else na_value_str, inline=True)

            current_turn_number_val = getattr(encounter, 'current_turn_number', na_value_str)
            current_index_val = getattr(encounter, 'current_index_in_turn_order', na_value_str)
            ...
            for enc in encounters:
                ...
                embed.add_field(
                    ...
                    value=field_value_template.format(
                        loc_id=str(enc.location_id) if enc.location_id else na_value_str,
                        turn=current_turn_number_val, # Already handles N/A
