...
from backend.core.crud.crud_item import item_crud # To validate item_id and get item names
from backend.core.crud.crud_player import player_crud # To validate owner
from backend.core.crud.crud_npc import npc_crud # To validate owner
...
from backend.models.inventory_item import InventoryItem # For type hinting
from backend.bot.utils import parse_json_parameter # Import the utility

logger = logging.getLogger(__name__)

class MasterInventoryItemCog(commands.Cog, name="Master Inventory Item Commands"): # type: ignore[call-arg]
    ...
    async def inventory_item_view(self, interaction: discord.Interaction, inventory_item_id: int):
        ...
        async with get_db_session() as session:
            ...
            async def get_label(key: str, default: str) -> str:
                return await get_localized_message_template(session, interaction.guild_id, f"inv_item_view:label_{key}", lang_code, default) # type: ignore
        ...
        async with get_db_session() as session:
            if owner_type:
                try: owner_type_enum = OwnerEntityType[owner_type.upper()]
                    ...
                    error_msg = await get_localized_message_template(session,interaction.guild_id,"inv_item_list:error_invalid_owner_type",lang_code,"Invalid owner_type.") # type: ignore
                    await interaction.followup.send(error_msg, ephemeral=True); return

            if owner_id is not None and owner_type_enum is None:
                error_msg = await get_localized_message_template(session,interaction.guild_id,"inv_item_list:error_owner_id_no_type",lang_code,"If owner_id is provided, owner_type must also be provided.") # type: ignore
                await interaction.followup.send(error_msg, ephemeral=True); return

            filters = [inventory_item_crud.model.guild_id == interaction.guild_id] # type: ignore
            if owner_id is not None and owner_type_enum:
                filters.append(inventory_item_crud.model.owner_entity_id == owner_id) # type: ignore
            ...
            if not inv_items:
                no_items_msg = await get_localized_message_template(session,interaction.guild_id,"inv_item_list:no_items_found",lang_code,"No InventoryItems found for {filter} (Page {p}).") # type: ignore
                await interaction.followup.send(no_items_msg.format(filter=filter_display, p=page), ephemeral=True); return

            title_tmpl = await get_localized_message_template(session,interaction.guild_id,"inv_item_list:title",lang_code,"InventoryItem List ({filter} - Page {p} of {tp})") # type: ignore
            ...
            footer_tmpl = await get_localized_message_template(session,interaction.guild_id,"inv_item_list:footer",lang_code,"Displaying {c} of {t} total items.") # type: ignore
            ...
            item_defs_dict: Dict[int, Any] = {} # Ensure type for item_defs_dict
            if item_ids_to_fetch:
                # Assuming item_crud.get_many_by_ids does not require guild_id for global items,
                # or handles context appropriately if items can be guild-specific.
                item_definitions = await item_crud.get_many_by_ids(session, ids=item_ids_to_fetch, guild_id=interaction.guild_id) # Pass guild_id for context
                item_defs_dict = {item_def.id: item_def for item_def in item_definitions}

            name_tmpl = await get_localized_message_template(session,interaction.guild_id,"inv_item_list:item_name_field",lang_code,"ID: {id} | Item: {name} (Base ID: {base_id})") # type: ignore
            val_tmpl = await get_localized_message_template(session,interaction.guild_id,"inv_item_list:item_value_field",lang_code,"Owner: {o_type}({o_id}), Qty: {qty}, Equipped: {eq}") # type: ignore
    ...
    async def inventory_item_create(self, interaction: discord.Interaction,
        ...
        async with get_db_session() as session:
            ...
            try:
                ...
                error_msg = await get_localized_message_template(session,interaction.guild_id,"inv_item_create:error_invalid_owner_type",lang_code,"Invalid owner_type. Use PLAYER or GENERATED_NPC.") # type: ignore
                await interaction.followup.send(error_msg, ephemeral=True); return
            ...
            try:
                async with session.begin():
                    created_inv_item = await inventory_item_crud.add_item_to_owner(
                        session=session, guild_id=interaction.guild_id, # guild_id is confirmed not None
                ...
                error_msg = await get_localized_message_template(session,interaction.guild_id,"inv_item_create:error_generic_create",lang_code,"Error adding item to inventory: {error}") # type: ignore
                await interaction.followup.send(error_msg.format(error=str(e)), ephemeral=True); return

            if not created_inv_item:
                error_msg = await get_localized_message_template(session,interaction.guild_id,"inv_item_create:error_unknown_fail",lang_code,"Failed to add item to inventory.") # type: ignore
                await interaction.followup.send(error_msg, ephemeral=True); return

            success_msg = await get_localized_message_template(session,interaction.guild_id,"inv_item_create:success",lang_code,"Item instance ID {id} (Qty: {qty}) added to {owner_type} {owner_id}.") # type: ignore
    ...
    async def inventory_item_update(self, interaction: discord.Interaction, inventory_item_id: int, field_to_update: str, new_value: str):
        ...
        if not field_type_info:
            async with get_db_session() as temp_session:
                error_msg = await get_localized_message_template(temp_session,interaction.guild_id,"inv_item_update:error_field_not_allowed",lang_code,"Field '{f}' not allowed. Allowed: {l}") # type: ignore
        ...
        async with get_db_session() as session:
            ...
            try:
                ...
                error_msg = await get_localized_message_template(session,interaction.guild_id,"inv_item_update:error_generic_update",lang_code,"Error updating InventoryItem {id}: {err}") # type: ignore
                await interaction.followup.send(error_msg.format(id=inventory_item_id, err=str(e)), ephemeral=True); return

            if not updated_inv_item:
                 error_msg = await get_localized_message_template(session,interaction.guild_id,"inv_item_update:error_unknown_update_fail",lang_code,"InventoryItem update failed.") # type: ignore
                 await interaction.followup.send(error_msg, ephemeral=True); return

            success_msg = await get_localized_message_template(session,interaction.guild_id,"inv_item_update:success",lang_code,"InventoryItem ID {id} updated. Field '{f}' set to '{v}'.") # type: ignore
        ...
        async with get_db_session() as session:
            ...
            try:
                ...
                if deleted_inv_item:
                    ...
                    error_msg = await get_localized_message_template(session,interaction.guild_id,"inv_item_delete:error_unknown_delete_fail",lang_code,"InventoryItem (ID: {id}) found but not deleted.") # type: ignore
                ...
                error_msg = await get_localized_message_template(session,interaction.guild_id,"inv_item_delete:error_generic_delete",lang_code,"Error deleting InventoryItem ID {id}: {err}") # type: ignore
