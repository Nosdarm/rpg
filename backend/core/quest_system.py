...
from backend.models import StoryLog
...
# from backend.core.ai_orchestrator import get_llm_evaluation_for_goal
...
async def _evaluate_abstract_goal(
    ...
    if evaluation_method == "llm_based":
        # from backend.core.ai_prompt_builder import prepare_quest_goal_evaluation_prompt
        # from backend.core.ai_orchestrator import get_llm_evaluation # Assuming a generic orchestrator function
...
async def _apply_quest_consequences(
    ...
    if "xp_award" in consequences_json:
        ...
        if isinstance(xp_amount, int) and xp_amount > 0:
            from backend.core.experience_system import award_xp
    ...
    if "relationship_changes" in consequences_json and isinstance(consequences_json["relationship_changes"], list):
        from backend.core.relationship_system import update_relationship
