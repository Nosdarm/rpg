...
class CRUDPartyNpcMemory(CRUDBase[PartyNpcMemory]):
    async def get_multi_by_party_and_npc(
        self,
        ...
        *,
        ...
        skip: int = 0,
        limit: int = 100
    ) -> Sequence[PartyNpcMemory]:
        """
        Get multiple memory entries for a specific party-NPC pair within a guild.
        """
        statement = (
            select(self.model)
            .where(
    ...
    async def get_multi_by_npc(
        ...
        *,
        ...
        """
        Get multiple memory entries for a specific NPC across all parties within a guild.
        """
        statement = (
            select(self.model)
            .where(
            ...
            .offset(skip)
            .limit(limit)
            .order_by(self.model.party_id, self.model.timestamp.desc()) # type: ignore
        )
        result = await session.execute(statement)
        return result.scalars().all()
    ...
    async def get_count_for_filters(
        ...
        *,
        ...
        if party_id is not None:
            conditions.append(self.model.party_id == party_id)
...
crud_party_npc_memory = CRUDPartyNpcMemory(PartyNpcMemory)
