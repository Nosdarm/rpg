...
async def _handle_move_action_wrapper(
    ...
    try:
        from backend.core.movement_logic import execute_move_for_player_action # Import here to avoid circular deps at module level if any
...
async def _handle_end_dialogue_action_wrapper(
    ...
    from backend.core.dialogue_system import end_dialogue # Local import
...
async def _execute_player_actions(
    ...
    from backend.models import StoryLog # For type hinting if needed, or load StoryLog entry
...
async def process_player_message_for_nlu(bot: commands.Bot, message: discord.Message, *, session: AsyncSession):
    ...
    try:
        ...
        if player.current_status == PlayerStatus.DIALOGUE:
            from backend.core.dialogue_system import handle_dialogue_input # Local import
